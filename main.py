...
# In-memory cache to prevent exceeding API rate limits (TTL: 5 minutes)
CACHE_TTL_SECONDS = 300
JOBS_CACHE_TTL_SECONDS = 180
...
# ==============================================================================
# TECH JOBS RADAR AGGREGATOR & ENDPOINT
# ==============================================================================
    ...
    async with httpx.AsyncClient(timeout=8.0) as client:
      ...
      if resp.status_code == 200:
        ...
        for item in items:
          ...
          # Filter to target Germany and Remote jobs
          if loc and re.search(r'\b(london|uk|united kingdom|wales|scotland|ireland|dublin|paris|france|spain|madrid|barcelona|usa|united states|canada|india)\b', loc, re.IGNORECASE):
            continue
  ...
  try:
    ...
    if df is not None and not df.empty:
      for _, row in df.iterrows():
        ...
        # Deduplicate against Arbeitnow listings
        if any(
...
async def get_jobs(
  ...
  try:
    ...
    return {
        ...
        "count": 0,
        "jobs": [],
        "message": f"Error aggregating jobs: {str(e)}",
    }
